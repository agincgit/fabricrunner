...
func runNative(ctx context.Context, config Config, command Command) (Result, error) {
	...
	profile := `(version 1)(deny default)(allow process-exec process-fork)(allow sysctl-read)(allow file-read-metadata)(allow file-read* (subpath "/usr") (subpath "/bin") (subpath "/System") (subpath "/Library/Apple") (literal "/dev/null") (literal "/dev/urandom") (subpath ` + strconv.Quote(config.Root) + `)) (allow file-write* (subpath ` + strconv.Quote(config.Root) + `))`
	if config.Helper != "" {
